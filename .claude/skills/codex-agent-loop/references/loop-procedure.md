...
- これらの subagent はリポジトリのファイルを編集しないため
  `isolation: "worktree"` は**付けない**（linked worktree を使うのは Codex
  実装 worker だけ。起票草案などの一時ファイル作成は repo 外なので
  読み取り専用扱いのまま）。
...
1. **着手可能なタスク集合の判定**
   ...
   - 集合が複数タスクの場合: 各タスクについて claim push と実装プランの記録
     ...
     分岐**して作成し、Codex 実装 worker を専用 linked worktree
...
2. **標準タスクフローの実行**（CLAUDE.md / docs/development-style.md に従う。
   ...
   - 独立サブ作業が 2 つ以上あれば Codex 実装 worker を専用 linked worktree で
     並列起動し、worker ごとの担当範囲・成果物の分担表をプランに書く。
   - 並列化しない場合は「並列化判定: 見送り（理由: …）」を明記する。判定の
     無記載はプラン不備としてレビューで差し戻す。
   - ブランチ `issue-<N>-slug` を main から作成し、テスト先行（red 確認 →
     green）で実装する。default branch（main）上では作業しない。実装は
     `gpt-5.6-sol` の Codex worker に委譲し、mainagent がレビューで収束させる。
   - **Codex worker の worktree を作る**: worker ごとにタスクブランチの同じ tip
     から linked worktree を作る。path は `.claude/worktrees/` 配下、branch は
     `worktree-agent-*` とし、作成前に `git worktree list --porcelain` と
     `git branch --list` で path / branch が未使用であることを確認する:
   ...
   - **prompt と返却先を repo 外へ作る**: `/tmp` 配下の task 固有ディレクトリに、
     worker 実行ごとに一意な prompt / result file を Write ツールで作る。prompt
     には Issue 本文、AC、実装プラン、担当範囲、変更可能なファイル、他 worker の
     分担、TDD の red → green、必要なローカル検証を含める。`commit` / `push` /
     `gh` / PR・Issue 操作 / worktree 操作を禁止し、担当外の既存変更を戻さず
     main/default branch を編集しないよう明記する。最終報告には変更ファイル、
     red の証拠、green の証拠、残課題、mainagent に確認してほしい判断を含める。
   - **bundled runner で起動する**:
     ...
     runner は model `gpt-5.6-sol`、sandbox `workspace-write`、ephemeral session、
     検証済み linked worktree を固定し、実行中は worktree を lock する。Codex の
     詳細ログは `<result-file>.log` に隔離する。成功時は result file だけを読み、
     ログを親コンテキストへ入れない。失敗時だけ runner が出す末尾の診断を読む。
     並列時は Bash の background execution を使い、foreground の sleep で待たず
     各 background task の完了通知を待つ。
   - **mainagent がレビューする**: result file と worktree の diff を mainagent
     自身が確認し、外部 Codex にレビューを委譲しない。指摘があれば同じ worktree
     に完全な修正 prompt と新しい result file を用意して runner を再実行する。
     各 runner 実行を `docs/development-style.md` 4.4.1 章の「実装 subagent
     試行回数」に 1 回として数える。
   - **Codex worker の成果の取り込みと worktree の復元**: worker には「commit /
     push はしない。ファイル変更のみ行い報告する」と指示しているため、成果は
     mainagent が worktree からパッチとして取り出してタスクブランチへ適用する。
     取り出したら**その場で worktree を元の状態へ戻す**（`<wt>` = worker の
   ...
   - この復元を省くと worker worktree は未コミットの変更を抱えたまま残り、
...
3. **CI 監視とマージ**
   ...
   - **CI red なら原因調査を読み取り専用 subagent に委譲する**（「調査
     ...
     3. 前回 red との同一性（同一失敗の反復か・失敗内容が変化したか。
        事実のみ返し、上限判定はしない）
     4. 推奨修正（どのファイルをどう直すか）
   - 親は返却された結論を基に修正して（必要なら Codex 実装 worker へ委譲し）再
...
- **`/code-review` の指摘を受け入れる流れ**: 全タスク完了時の最終レポートを
  ...
  章）で次イテレーションに選ばれるため、 ユーザーが `/codex-agent-loop`
  を再開すればループが指摘を処理する。
