...
// #385: 羊皮紙の色定数は npm 非依存の純粋モジュールへ切り出した。パレット生成
// （scripts/build-colors.ts）が陸地色を引くのに basemap.ts 経由（= @protomaps/
// basemaps 依存）を強いられないようにするため。従来の参照点は re-export で保つ。
import {
  PARCHMENT_FLAVOR_OVERRIDES,
  PARCHMENT_LANDCOVER_COLORS,
} from "./parchment_palette.ts";

export { PARCHMENT_FLAVOR_OVERRIDES, PARCHMENT_LANDCOVER_COLORS };
