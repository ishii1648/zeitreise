...
### 3.14 沿岸補完の帯（`coastal_fill_<year>.geojson`）の既知の穴（#358）
...
`europe_<year>.geojson` から機械的に導いた表示用の面である。片側オフセットを 1
本の環（`src/coastal_fill.ts` `coastalBandPolygon`）で表す構成のため、
**海岸線が帯幅より細かい刻みでジグザグする区間ではオフセット列が折り返し、
巻き数が打ち消し合ったポケットが帯の穴として残る**。

全 19 年代 × 全宗主キーで勢力圏の外枠（`hre-extent`）が持つ内環 2,421 本のうち
1,733 本は base ポリゴンの頂点を 1 つも含まない（= 帯だけで囲まれた環）で、
うち平均半幅 500 m 以上が 750 本ある。ほとんどは沖合にあり、ベースマップの 海洋
`water` に覆われるため画面には出ない。

画面に出る唯一の実例が**クロニアン砂州沖の 2 環**で、東プロイセンの海岸線が
年代間で使い回されているため **19 年代のうち 17 年代**に同じ形で現れる（1000 年
Prussians / 1279〜1500 年 Teutonic Knights / 1530〜1815 年 Prussia / 1880・1900
年 Germany / 1914 年 German Empire）。

| 環                  | 面積      | 周長     | 平均半幅  | bbox                            | 現代の陸に乗る長さ |
| ------------------- | --------- | -------- | --------- | ------------------------------- | ------------------ |
| 潟の北側（頂点 10） | 188.7 km² | 75.22 km | 2,508.3 m | `[20.746,55.248,21.067,55.520]` | 0.00 km（0.0%）    |
| 砂州側（頂点 6）    | 62.2 km²  | 56.70 km | 1,096.7 m | `[20.754,55.101,20.944,55.327]` | 7.75 km（13.7%）   |

後者の露出区間（`[20.754,55.101]` を頂点とする V 字、bbox
`[20.754,55.101,20.794,55.149]`）だけが現代の砂州の陸側に出るため、その勢力を
ホバー/選択したとき **z7 で約 22 px の孤立した臙脂線**として見える（z5 では 1.8
× 3.8 px で点にしか見えない）。**これは既知の残差であり新規の回帰では ない。**
臙脂線は帯の穴の縁と頂点集合まで一致する = 見えている緑青の塗りの縁
そのものなので、#330 AC4 が禁じた「表示境界と一致しない概略海岸線」には当たら
ない。判断根拠と各案の検討は
[../research/issue-358-suzerain-extent-inner-ring.md](../research/issue-358-suzerain-extent-inner-ring.md)、
現状の環構成は `src/suzerain_extent_coastal_test.ts` の特性テストが固定して
いる。是正するなら帯のオフセット構成そのものを変える（穴を作らない形へ）話に
なり、19 年代の帯の作り直しを伴う。
