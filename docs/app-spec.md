...
- 重ね順（TASK-77。判定ロジックと根拠は `src/layer_stack.ts`）:
  ...
  - ラベル層と都市の点・ヒット円・選択リングは interleaved ではなく overlaid
    ...
    衝突で地名が消えた都市の円とヒット判定は無効にし、地名の再表示と同期して戻す。
...
- **対応（#304）**: Pages 側中間層を顧客が直接パージする手段は無い（公式の
  ...
  毎デプロイの URL 単位パージでエッジ側の再固定を剥がす対応を行った。中間層の
  ...
  実測できたら削除してよい。#314 の 2026-09-05 本番実測で全 6 件の 404 が
  確認されたため、#485 で該当行を削除した
...
- 隣接勢力の色衝突はパレット設計（十分な色数・彩度差）で緩和し、MVP
  では厳密な四色問題的解決はしない
- パレットのスロットは**羊皮紙ベースマップの陸地色に対する知覚色差**で絞る
  （Issue #385・ADR-0041）。塗りを `FILL_ALPHA` で陸地色 `#f0e6cd` に合成した
  実表示色と素の陸地色の CIEDE2000 色差が `EARTH_DELTA_E_MIN`（10）未満の
  スロットは割当候補から除外する。属領の派生色（明度シフト後）も同じ基準を
  満たすスロットだけを使う。パレットの (h,s,l) 格子自体（`SATURATIONS` /
  `LIGHTNESSES`）は変えていない

#### 政治ラベルの表示階層

表示上の「国名」は出典 GeoJSON の `base` feature
名を意味しない。政治的に独立した
国家名の一般語であり、ラベル選択では次の構造的な 3 階層へ分類する。

- **上位政治圏**: 帝国・王国など、概観で領域全体を代表する単位
- **構成国**: 上位政治圏の直下にある主要な国家・勢力。データ上は `SUBJECTO` /
  `PARTOF` が同じ最上位勢力を指すオーバーレイ feature
- **領邦**: 構成国より下位の個別単位。`SUBJECTO` と `PARTOF` が異なることで 2
  段以上の所属が宣言された feature。構造が不足するデータは安全側として
  構成国へ分類し、名称の「王国」「公領」「伯領」等から歴史的序列を推測しない

ズーム別の候補選択は、z4 では上位政治圏のみ、z5〜6 では上位政治圏を残して 面積
priority が機械的なしきい値を満たす主要構成国を追加、z7〜8 では全 3 階層を
候補にする。上位政治圏名は文字・halo が代表領土内に収まる位置だけを採用し、 18px
の1行、意味の区切りで最大2行、下限14pxの順に配置を試みる。収まらない
縮尺では省略し、整数ズーム段ごとに再判定する。領土外への退避と引き出し線は
使用しない。省略時も領土のホバー・クリックで正式名称を確認できる。
候補同士の最終的な取捨は階層 priority と衝突制御に従う。

1715 / 1783 / 1800 年の神聖ローマ帝国は `base` 区分を上位政治圏の根拠にしない。
#332 の OpenHistoricalMap 由来 `hre_realm_<year>.geojson`
から「神聖ローマ帝国」を 1 件生成する。各 base feature は Polygon 成分ごとの
polylabel アンカーを同じ帝国全域で包含判定し、帝国外にアンカーを
持つ成分だけを上位ラベル候補にする。帝国内の構成国・領邦名は HRE
オーバーレイから生成するため、 オーストリア・プロイセン等の feature
全体を一律に帝国所属へせず、ハンガリーや 東プロイセンを HRE
ラベル範囲へ取り込まない。`hre_realm` の無い年代は従来規則へ
縮退し、帝国解体後の 1815 年以降に HRE 名を合成しない。参考画像の座標・形状は
ラベル位置または所属判定に使用しない。
...
- ホバー: 勢力名（`NAME`、`SUBJECTO` があれば「NAME — SUBJECTO
  領」）をツールチップ表示
- クリック / タップ: 勢力・河川・山脈・山峰の選択強調を切り替える。名称や説明を
  載せたパネルは表示しない（Issue #423）。都市や地図上の空白をクリックしても
  説明 UI は表示しない
- タッチ主体の環境では、タップに伴う疑似 hover でカーソル追従ツールチップが
  残らないよう抑止する。選択強調はクリック処理で従来どおり更新する
- 年代別の勢力説明データは将来の再設計に備えてリポジトリ内の開発者向け資料として
  保持するが、クライアントは取得・保持・表示しない
- 出典・ライセンスの全文は右下のアトリビューション（ⓘ）から確認できる。
  FeatureCollection の `metadata` と生成時の帰属情報は維持する
- **HRE主要領邦の3段階表現（#444 / ADR-0043）**: `data/hre-major-polities.json`
  の各対象を、同年の通常面 → 静的許可リストの隣接年借用面 →
  境界未収録マーカーの順にちょうど1状態へ
  解決する。通常面が後から加われば借用面とマーカーは自動抑制する。自動最近傍年
  検索や出典のない面の合成はしない
  - 借用面は通常面より低alphaで描き、ホバーまたはクリック時のツールチップに
    「近似」、借用元年、feature単位の出典・ライセンス・借用理由を表示する
  - 借用不能時は `data/cities.json` の中心都市に単色の旗形記号と勢力名を置く。
    画面内に対象中心地がある間は「旗形＝勢力の中心地（領域は未表示）」の凡例を
    地図上端に表示する。ホバー/クリック時には領邦名、称号、存続期間、宗主、中心都市、
    欠落理由、known-limitations IDを表示する。点は領域や帰属を主張せず、通常都市
    マーカーも置き換えない
  - Issue #423 の「通常のクリックで説明UIを出さない」規則に対し、近似・未収録の
    不確かさを常時追跡可能にするこの2表現だけはクリックでも同じツールチップを出す
...
- **諸侯領の第 2 の出典（TASK-110、decision-26）**: OHM 由来の諸侯領には年代・
  ...
  描かれることはない
  - #450: モルダヴィア公国は1400〜1800年をCliopatriaの年代別面で補完する。
    1492年以後は`SUBJECTO` / `PARTOF = Ottoman Empire`として宗主関係を示す。
    1600〜1601年の三公国合成面は政体モデルと両立しないため除外し、直前の
    1595〜1599年面を座標無改変・`BORROWED_FROM`付きで借用する。1815年は
    同年を直接包含するOHM relation 2694163を優先する（ADR-0044）
...
- **山岳（TASK-100）**: 山脈は**名称のみ**（`アルプス山脈`）、山峰は**名称 +
  標高**（`モンブラン 4807m`）をホバーで出す。山脈のポリゴンは標高を
  持たず、山脈に単一の標高という概念も無い（それは山峰の属性）ため。地図上の
  山峰ラベルは衝突ボックスを小さく保つため z7 未満で標高を隠すが、ツールチップ
  は衝突空間の外なので常に標高を出す。山岳は年代非依存の地形なので、
  **年代を切り替えても内容は変わらない**（整形関数が year を引数に取らないこと
  をテストで固定している）
- **都市（Issue #221 / #222）**: **名称 + 人口**（`パリ 人口約200,000人`）を
  ホバーで出す（`src/cities.ts` の `cityPickLabel`。山峰の `peakPickLabel`
  と同型の純粋関数）。人口不明（`population: null`）は名称のみ。
...
- picking の優先順位は `src/picking.ts` の `PICKING_PRIORITY`（河川 > 都市 >
  山峰 > 都市ヒット層 > 山峰ヒット層 > 山脈ヒット層 > 河川ヒット層 >
  境界未収録マーカー > HRE 領邦 > 仏諸侯領 > 伊諸侯領 > 勢力）で一元管理し、
  deck.gl のレイヤー描画順（配列の後ろほど上 =
