...
- **勢力圏の外枠（TASK-30 / TASK-94 / TASK-120。実装は
  ...
  - 帯を入力に足すのは、画面上でその勢力の面として塗られるのが「元ポリゴン +
    ...
    `water` が覆うため）
  - **既知の残差（#358）**: 帯自身が持つ穴は外枠の内環としてそのまま残るため、
    そこでは臙脂線が領域の内部に線として出る。実例はクロニアン砂州沖の 2 環で、
    東プロイセンを含む勢力を選ぶと 19 年代のうち 17 年代で現れ、現代の砂州の
    陸側に出る 7.75 km（z7 で約 22px・z5 では点）だけが見える。この線は帯の穴の
    縁と頂点集合まで一致する = 見えている緑青の塗りの縁そのものなので、上の
    「一致させない区間では概略海岸線を残さない」に反していない（同じ臙脂線を
    見たときに新規の回帰と取り違えないこと）。詳細と各案の検討は
