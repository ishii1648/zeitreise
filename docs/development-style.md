...
**既存 ADR の記述が実態と乖離したときの扱い**（#336 で規約化）: ADR は決定の
記録なので「下した決定を無かったことにする書き換え」はしない。一方で、決定は
そのままなのに ADR が古い値を主張し続けるのも同じくらい有害である（読み手が ADR
を信じて実装と食い違う判断をする）。**乖離の性質で 2 つに分ける**:

| 乖離の性質                                                                                         | 扱い                                                                                                                                               | 前例                                                                                                   |
| -------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| **決定そのものが変わる**（方式・原則・採否の変更、条件の追加・緩和）                               | 新しい ADR を起こす（追補、または置き換え）。元 ADR の Decision は書き換えず相互参照し、置き換えの場合は元 ADR の `status` を `superseded` にする  | ADR-0035（ADR-0033 の追補。借用面の表示方法を追補として決め直した）                                    |
| **決定は変わらず、その決定が定めた規則の適用結果だけが変わる**（パラメータ値・対象リスト・実測値） | 元 ADR の該当箇所を現状値へ**改訂**し、旧値・変更契機（Issue / TASK 番号）をその場に注記する。`status` は `accepted` のまま。新規 ADR は起こさない | ADR-0003（河川の scalerank を 5 → 6 へ、TASK-152）、ADR-0026（1300 年のブロワ伯領の除外、#321 / #336） |

判断は「読み手が別の選択肢を選びうる判断が新たに下されたか」で行う。下されて
いなければ改訂（後者）で足りる。加えて:

- ADR に書き写すのは**判断基準**が原則で、コードが持つ値・対象リストの写しは
  最小限にする。写す場合は「正はコード側」と ADR 本文に明記する。
- 写しを置いたなら**乖離の再発を検出するテストを同時に置く**（例:
  `scripts/build-cliopatria-fiefs_test.ts` が ADR-0026 の適用範囲の表を
  パースして許可リストと突き合わせる）。人手の同期に頼らない。
...
- `data/known-limitations.json`（開発者向けの記録。#328 でユーザー向け表示は
  撤去したが、欠落の所在と根拠を残す台帳としての役割は変わらない）
