...
import type { DebugHooksTarget } from "../../src/debug_hooks.ts";
...
export async function run(api: CdpApi) {
  ...
  const probes = await api.evaluate<{
    name: string;
    center: ReturnType<NonNullable<DebugHooksTarget["__probePick"]>>;
    edge: ReturnType<NonNullable<DebugHooksTarget["__probePick"]>>;
